...
import planmodel from "../model/planSchema.js";
...
export const totalAppoinmtent = async (req: Request, res: Response) => {
  try {
    const maleCount: number = await APPOINTMENT.countDocuments({
      gender: "male",
    });
    const femaleCount: number = await APPOINTMENT.countDocuments({
      gender: "female",
    });
...
export const addPlans = async (req: Request, res: Response) => {
  try {
    const planData = {
      ...
      number_of_chamber: req.body.number_of_chamber,
      ...
      number_of_staff: req.body.number_of_staff,
      max_video: req.body.max_video,
    ...
    if (!req.body.planId) {
      ...
      const result = await PLAN.updateOne({ _id: req.body.planId }, planData);
      res
        .status(200)
...
export const getPlans = async (req:Request, res: Response) =>  {
  ...
  res.status(200).send({ success: true, message: "here are your plans",plans });
}
...
export const deletePlanById = async (req: Request, res: Response) => {
  ...
  res.status(200).send({ success: true, message: "plan deleted successfully",result });
}